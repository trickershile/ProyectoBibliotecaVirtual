...
const Button = ({ children, variant = "primary", onClick, className = "", to, type = "button" }) => {
  ...
  const classes = `px-6 py-2 rounded-lg border font-mono text-sm font-bold tracking-tighter transition-all duration-300 active:scale-95 flex items-center justify-center space-x-2 group ${resolvedVariant} ${className}`;
